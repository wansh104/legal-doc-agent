...
from src.pipeline import run_pipeline, render_report_markdown, StageError, inject_test_error
...
if run_clicked:
    ...
    # "inject test error" button below — a fresh button click re-runs this
    # whole script, and a local variable wouldn't persist across that.
    st.session_state["pipeline_result"] = result
    st.session_state.pop("corrupted_report", None)
    st.success("Done.")
 
if "pipeline_result" in st.session_state:
    result = st.session_state["pipeline_result"]
    output_path = result.docx_path
    ...
    st.divider()
    st.subheader("3. Prove the checks actually catch errors")
    st.caption(
        "Makes a copy of the generated document with a deliberately wrong respondent "
        "number, then re-runs evaluation against it. No new LLM calls — reuses the same "
        "extracted facts and drafted paragraphs from above."
    )
    if st.button("Inject a test error and re-evaluate"):
        corrupted_path = os.path.join(tempfile.gettempdir(), "corrupted_demo_affidavit.docx")
        corrupted_report = inject_test_error(result, corrupted_path)
        st.session_state["corrupted_report"] = corrupted_report
        st.session_state["corrupted_path"] = corrupted_path
 
    if "corrupted_report" in st.session_state:
        cr = st.session_state["corrupted_report"]
        st.metric("Score after corruption", f"{cr.overall_score}/100",
                   delta=f"{round(cr.overall_score - r.overall_score, 1)}")
        for issue in cr.issues:
            icon = "\U0001F534" if issue.severity.value == "error" else "\U0001F7E1"
            st.markdown(f"{icon} **[{issue.dimension}]** {issue.message}  \n*Source: {issue.source}*")
        with open(st.session_state["corrupted_path"], "rb") as f:
            st.download_button(
                "Download corrupted_demo_affidavit.docx (for inspection)", f,
                file_name="corrupted_demo_affidavit.docx",
                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            )
